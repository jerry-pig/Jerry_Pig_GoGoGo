...
试计算二重积分：
\[
  I = \iint_D \frac{x^3 + \sin(x y^2) + x^2 y}{x^2 + y^2}\,dxdy
\]
\textit{提示：注意观察区域 $D$ 的对称轴与被积函数各项的奇偶性、轮换对称性，合理拆项与转化坐标系，避免强行暴力积分。}
\end{problem}
...
\textbf{Strategy:}\enspace 被积函数拆为三项。利用区域 $D$ 关于直线 $y = x$ 的对称性（轮换对称），对调 $x, y$ 积分值不变；同时极坐标转换计算代数项。

\vspace{1.5mm}
\textbf{Derivation:}\enspace
区域 $D$ 是关于 $y = x$ 对称的。对于第二项 $\sin(xy^2)$，在 $D$ 内不具有奇偶消去性，但如果原本题目含奇偶性项，直接识别。在本题中，考虑主项拆分：
\[
  I = \iint_D \frac{x^3 + x^2 y}{x^2+y^2}dxdy + \iint_D \frac{\sin(xy^2)}{x^2+y^2}dxdy
\]
由轮换对称性，$x \leftrightarrow y$：$\iint_D \frac{x^3}{x^2+y^2}dxdy = \iint_D \frac{y^3}{x^2+y^2}dxdy$。两式相加折半：
\[
  \iint_D \frac{x^3+y^3}{x^2+y^2}dxdy = \frac{1}{2}\iint_D \frac{(x+y)(x^2-xy+y^2)}{x^2+y^2}dxdy
\]
更直接地，转极坐标 $x = r\cos\theta, y = r\sin\theta$，$D: 0 \le r \le 1, \, 0 \le \theta \le \frac{\pi}{2}$：
\[
  \iint_D \frac{x^3 + x^2 y}{x^2+y^2} dxdy = \int_0^{\pi/2} (\cos^3\theta + \cos^2\theta\sin\theta)\,d\theta \int_0^1 \frac{r^3}{r^2} \cdot r dr = \left(\frac{2}{3} + \frac{1}{3}\right) \cdot \frac{1}{3} = \frac{1}{3}
\]
\begin{warnbox}
\alerttag{Common Error}\enspace 轮换对称性 $x \leftrightarrow y$ 要求积分区域 $D$ 必须自身关于直线 $y = x$ 严格对称！若区域不对称，切忌直接对调。
\end{warnbox}
\end{solution}
