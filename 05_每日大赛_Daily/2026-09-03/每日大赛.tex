...
\begin{enumerate}[leftmargin=1.5em,itemsep=1pt,topsep=1pt]
  \item 将 $f(x)$ 表为对导数 $f'(t)$ 的积分形式，并利用 Cauchy-Schwarz 不等式证明：$[f(x)]^2 \le x \int_0^x [f'(t)]^2 dt$；
  \item 利用分部积分法，严格证明不等式：
  \[
    \int_0^1 [f(x)]^2\,dx \le \frac{1}{2} \int_0^1 (1-x^2)[f'(x)]^2\,dx \le \frac{1}{2} \int_0^1 [f'(x)]^2\,dx.
  \]
\end{enumerate}
\end{problem}
...
(1) 令 $x=0$ 代入得 $f(0) = 0$。\\
(2) 拆项求一阶导：$f'(x) = 1 + 2f(x) - [\int_0^x f(t)dt + xf(x)] + xf(x) = 1 + 2f(x) - \int_0^x f(t)dt \implies f'(0)=1$。\\
求二阶导：$f''(x) = 2f'(x) - f(x) \implies f''(x) - 2f'(x) + f(x) = 0$。\\
(3) 特征方程 $(r-1)^2 = 0 \implies r_1=r_2=1$。通解 $f(x)=(C_1+C_2 x)e^x$。代入初值 $f(0)=0, f'(0)=1 \implies f(x) = x e^x$。
\end{solution}

\vspace{1.5pt}

\begin{solution}{M2 \textperiodcentered\ Pumping Work from Hemispherical Tank}
(1) 在深度 $x \in [0, R]$ 处切薄圆盘，半径 $r(x) = \sqrt{R^2 - x^2}$。微质量 $dm = \rho\pi(R^2 - x^2)dx$。\\
(2) 提升高度为 $x$，微功 $dW = x\cdot g dm = \pi\rho g (R^2 x - x^3)dx$。\\
总功 $W = \int_0^R dW = \pi\rho g \left[\frac{R^2 x^2}{2} - \frac{x^4}{4}\right]_0^R = \frac{1}{4}\pi\rho g R^4$。
\end{solution}

\vspace{1.5pt}

\begin{solution}{M3 \textperiodcentered\ Cauchy-Schwarz Integral Inequality}
(1) $f(x) = \int_0^x 1\cdot f'(t)dt$。在 $[0, x]$ 上应用 Cauchy-Schwarz：$[f(x)]^2 \le (\int_0^x 1^2 dt)(\int_0^x [f'(t)]^2 dt) = x \int_0^x [f'(t)]^2 dt$。\\
(2) 对 $x$ 在 $[0, 1]$ 上分部积分（令 $u=\int_0^x [f'(t)]^2 dt, dv=x dx \implies v=x^2/2$）：\\
$\int_0^1 [f(x)]^2 dx \le \left[\frac{x^2}{2}\int_0^x [f'(t)]^2 dt\right]_0^1 - \int_0^1 \frac{x^2}{2}[f'(x)]^2 dx = \frac{1}{2}\int_0^1 (1-x^2)[f'(x)]^2 dx \le \frac{1}{2}\int_0^1 [f'(x)]^2 dx$。证毕！
\end{solution}

\vspace{1.5pt}

\begin{solution}{Q1 \textperiodcentered\ Rectangular Barrier Penetration \& Transmission}
(1) $\psi_I = e^{ikx} + R'e^{-ikx}$, $\psi_{II} = Ce^{-\kappa x} + De^{\kappa x}$, $\psi_{III} = T'e^{ikx}$，其中 $k = \frac{\sqrt{2mE}}{\hbar}, \kappa = \frac{\sqrt{2m(V_0-E)}}{\hbar}$。\\
(2) 在 $x=0, a$ 处匹配 $\psi$ 与 $\psi'$ 连续，消去 $R', C, D$ 取模方得 $T = |T'|^2 = \left[1 + \frac{V_0^2}{4E(V_0-E)}\sinh^2(\kappa a)\right]^{-1}$。\\
(3) 几率流 $J_{\text{ref}} = \frac{\hbar k}{m}|R'|^2, J_{\text{trans}} = \frac{\hbar k}{m}|T'|^2 \implies R + T = |R'|^2 + |T'|^2 = 1$（定态几率流守恒）。
\end{solution}

\vspace{1.5pt}

\begin{solution}{Q2 \textperiodcentered\ Gamow Factor \& Resonant Transmission}
(1) $\kappa a \gg 1$ 时 $\sinh^2(\kappa a) \approx \frac{1}{4}e^{2\kappa a} \implies T \approx \frac{16E(V_0-E)}{V_0^2} e^{-2\kappa a}$（透射几率随势垒厚度指数衰减）。\\
(2) $E > V_0$ 时 $\kappa \to -ik_2 \implies \sinh^2(\kappa a) \to -\sin^2(k_2 a)$。当 $\sin(k_2 a) = 0$ 即 $k_2 a = n\pi$ ($a = n\frac{\lambda_2}{2}$) 时，$T = 1, R = 0$ 发生共振完全透射。
\end{solution}
