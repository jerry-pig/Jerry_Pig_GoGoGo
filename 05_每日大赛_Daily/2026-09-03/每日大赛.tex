...
(1) 由 $f(0)=0$，由微积分基本定理得 $f(x) = \int_0^x 1 \cdot f'(t)dt$。在局部区间 $[0, x]$ 上应用 Cauchy-Schwarz 不等式：\\
$[f(x)]^2 = \left(\int_0^x 1 \cdot f'(t)dt\right)^2 \le \left(\int_0^x 1^2 dt\right) \left(\int_0^x [f'(t)]^2 dt\right) = x \int_0^x [f'(t)]^2 dt$。\\
(2) 两边在 $[0, 1]$ 上对 $x$ 积分：$\int_0^1 [f(x)]^2 dx \le \int_0^1 x \left(\int_0^x [f'(t)]^2 dt\right) dx$。\\
应用分部积分法，令 $u = \int_0^x [f'(t)]^2 dt$，$dv = x\,dx \implies v = \frac{x^2}{2}$：\\
$\int_0^1 x \left(\int_0^x [f'(t)]^2 dt\right) dx = \left[ \frac{x^2}{2} \int_0^x [f'(t)]^2 dt \right]_0^1 - \int_0^1 \frac{x^2}{2} [f'(x)]^2 dx = \frac{1}{2}\int_0^1 (1-x^2)[f'(x)]^2 dx$。\\
由于在 $x \in [0, 1]$ 上恒有 $1-x^2 \le 1$，且 $[f'(x)]^2 \ge 0$，因此：\\
$\frac{1}{2}\int_0^1 (1-x^2)[f'(x)]^2 dx \le \frac{1}{2}\int_0^1 [f'(x)]^2 dx$。全式证毕！
\end{solution}
