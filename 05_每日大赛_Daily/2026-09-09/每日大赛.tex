...
设定义在区间 $[0, +\infty)$ 上的连续函数 $f(x)$ 满足积分方程 $f(x) = \sin x + \int_0^x (x-t)f(t)\,dt$。
...
计算下列 $n$ 阶三对角行列式 $D_n$（主对角元均为 5，次对角元均为 2）：
\[
  D_n = \begin{vmatrix}
    5 & 2 & 0 & \dots & 0 \\
    2 & 5 & 2 & \dots & 0 \\
...
\begin{enumerate}[leftmargin=1.5em,itemsep=0.5pt,topsep=1pt]
  ...
  \item 不计自旋时利用等差求和证明主量子数 $n$ 能级简并度 $g_n = \sum_{l=0}^{n-1} (2l+1) = n^2$；计入自旋为何是 $2n^2$？简述“偶然简并”物理根源；
  \item 设氢原子处于归一化定态叠加态 $\psi = \frac{1}{\sqrt{3}}\psi_{200} + \sqrt{\frac{2}{3}}\psi_{211}$：(a) 测量 $\hat{L}^2$ 与 $\hat{L}_z$ 分别可能得到什么确定值及对应几率？(b) 计算角动量分量期望值 $\langle L_z \rangle$。
\end{enumerate}
\end{problem}
...
一阶求导（乘积法则）：$f'(x) = \cos x + \left[1\cdot\int_0^x f(t)dt + xf(x)\right] - xf(x) = \cos x + \int_0^x f(t)dt$（前后 $xf(x)$ 必对消！）。\\
二阶求导：$f''(x) = -\sin x + f(x) \implies f''(x) - f(x) = -\sin x$。证毕！\\
(2) 初值：令 $x=0$ 得 $f(0) = \sin 0 + 0 = 0$；在一阶导数式中令 $x=0$ 得 $f'(0) = \cos 0 + 0 = 1$。\\
(3) 特征方程 $r^2 - 1 = 0 \implies r = \pm 1$，齐次通解 $y_h = C_1 e^x + C_2 e^{-x}$。\\
设特解 $y^* = A\cos x + B\sin x$，代入得 $(-2A)\cos x + (-2B)\sin x = -\sin x \implies A=0, B=1/2$，即 $y^* = \frac{1}{2}\sin x$。\\
通解 $f(x) = C_1 e^x + C_2 e^{-x} + \frac{1}{2}\sin x$。代入初值：$f(0) = C_1 + C_2 = 0 \implies C_2 = -C_1$；$f'(0) = 2C_1 + 1/2 = 1 \implies C_1 = 1/4, C_2 = -1/4$。\\
...
\small
(1) 按第一行展开：$D_n = 5 M_{11} - 2 M_{12}$。其中 $M_{11} = D_{n-1}$；余子式 $M_{12}$ 第一列仅含主元 2，按该列展开得 $2 D_{n-2}$。\\
因此递推关系为：$D_n = 5D_{n-1} - 4D_{n-2}$（$n \ge 3$）。\\
(2) 初值计算：$D_1 = |5| = 5$；$D_2 = \begin{vmatrix} 5 & 2 \\ 2 & 5 \end{vmatrix} = 25 - 4 = 21$。\\
(3) 特征方程 $r^2 - 5r + 4 = 0 \implies (r-1)(r-4) = 0$，特征根为 $r_1 = 1, r_2 = 4$。通解形式 $D_n = c_1 + c_2 4^n$。\\
代入初始条件：$\begin{cases} c_1 + 4c_2 = 5 \\ c_1 + 16c_2 = 21 \end{cases} \implies 12c_2 = 16 \implies c_2 = \frac{4}{3}, \quad c_1 = -\frac{1}{3}$。\\
通项显式闭式解：$D_n = -\frac{1}{3} + \frac{4}{3}\cdot 4^n = \frac{4^{n+1} - 1}{3}$。（自检：$n=1 \implies 5$；$n=2 \implies 21$；$n=3 \implies 85 = 5(21)-4(5)$，完全正确！）
\end{solution}
...
$\langle r \rangle = \int_0^\infty r P(r) dr = \frac{4}{a_0^3} \int_0^\infty r^3 e^{-2r/a_0} dr$。令 $t = \frac{2r}{a_0} \implies \langle r \rangle = \frac{4}{a_0^3} (\frac{a_0}{2})^4 \int_0^\infty t^3 e^{-t} dt = \frac{a_0}{4} \cdot 3! = \frac{3}{2}a_0$。\\
(2) 最概然半径 $r_p = a_0$ 对应密度峰值 $\frac{dP}{dr}=0$；而平均半径 $\langle r \rangle = 1.5 a_0$ 是重心期望。几率密度分布在 $r \in [0, \infty)$ 上不对称，外侧存在指数渐近长尾（Long Tail），将重心向外拉移，故 $\langle r \rangle > r_p$。\\
(3) 势能期望值：$\langle V \rangle = \int_0^\infty (-\frac{e^2}{r}) P(r) dr = -\frac{4e^2}{a_0^3} (\frac{a_0}{2})^2 \int_0^\infty t e^{-t} dt = -\frac{e^2}{a_0} \cdot 1! = -\frac{e^2}{a_0}$。\\
因基态总能 $E_1 = -\frac{e^2}{2a_0} = \langle T \rangle + \langle V \rangle$，得动能 $\langle T \rangle = E_1 - \langle V \rangle = +\frac{e^2}{2a_0} > 0$。显见 $\langle T \rangle = -\frac{1}{2}\langle V \rangle$，维里定理严格成立！
\end{solution}
...
\small
(1) 量子数取值：轨道角动量 $l = 0, 1, 2, \dots, n-1$（共 $n$ 个）；磁量子数 $m = -l, -l+1, \dots, +l$（共 $2l+1$ 个）。\\
(2) 简并度求和：$g_n = \sum_{l=0}^{n-1} (2l+1) = 2\cdot\frac{(n-1)n}{2} + n = n(n-1) + n = n^2$。计入自旋 $s=1/2$（$m_s = \pm 1/2$）后总简并度乘 2 为 $2n^2$。\\
库仑势能级与 $l$ 无关为“偶然简并”，本质是由 Runge-Lenz 矢量守恒决定的更高阶 $SO(4)$ 动力学对称性。\\
(3) (a) 态矢由 $|2,0,0\rangle$（几率 $1/3$）与 $|2,1,1\rangle$（几率 $2/3$）叠加而成。\\
测量 $\hat{L}^2$：可能测得 $0\hbar^2=0$（几率 $1/3$）或 $1(1+1)\hbar^2=2\hbar^2$（几率 $2/3$）；\\
测量 $\hat{L}_z$：可能测得 $0\hbar=0$（几率 $1/3$）或 $1\hbar=\hbar$（几率 $2/3$）。\\
(b) 期望值：$\langle L_z \rangle = \frac{1}{3}(0) + \frac{2}{3}(\hbar) = \frac{2}{3}\hbar$。
\end{solution}
