...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \begin{scope}[shift={($(current page.center)+(0,1.2cm)$)}]
    ...
    \draw[line width=0.7pt, royal!65, -{Stealth[length=2.5mm]}] (0,-3.6) -- (0,3.6)
      node[above, font=\footnotesize\sffamily\color{mist}] {$V_{\text{eff}}(r),\, P(r)$};

    % Hydrogen radial probability density P(r) = 4 * r^2 * exp(-2r)
    \draw[line width=1.4pt, royal, smooth, samples=120, domain=0:3.8]
      plot ({\x}, {3.5*\x*\x*exp(-1.8*\x)});

    % Effective potential curve V_eff(r)
    \draw[line width=1.2pt, amber!90!black, smooth, samples=100, domain=0.6:3.8]
      plot ({\x}, {1.8/(\x*\x) - 3.2/\x});

    % Dashed vertical drop to minimum of V_eff
    \draw[line width=0.6pt, forest, dashed] (1.125,-1.42) -- (1.125, 0);

    \fill[navy] (0,0) circle (3.5pt);
    \fill[amber] (1.125, -1.42) circle (2.2pt);
    \fill[royal] (1.11, 1.88) circle (2.2pt);

    \node[above, font=\footnotesize\sffamily\color{royal}] at (1.11, 1.95) {$r_p = a_0$};
    \node[below right, font=\footnotesize\sffamily\color{amber!90!black}] at (1.18, -1.42) {$\min V_{\text{eff}}$};
  \end{scope}
  ...
  \node[anchor=center] at ($(current page.center)+(0,-3.4cm)$) {
    \begin{minipage}{0.84\textwidth}
      ...
      {\fontsize{11}{14}\selectfont\sffamily\bfseries\color{royal}%
        GRAND ARENA OF THEORETICAL PHYSICS \textperiodcentered\ MULTIVARIABLE \& HYDROGEN}\\[10pt]
      {\color{navy}\rule{0.45\textwidth}{1.2pt}}\\[8pt]
      {\fontsize{10}{13}\selectfont\color{slate}\itshape
        ``604 Multivariable Calculus: Hessian Discriminant $\times$ Lagrange Volume Optimization\\
        811 Quantum Mechanics: Symmetric Well Parity $\times$ Hydrogen Radial Stability''\\[3pt]
        {\small\color{mist}\upshape 5-Problem Intensive Battle \textperiodcentered\ Closed-book \textperiodcentered\ 80\,min Real-time Challenge}}
    \end{minipage}
  };
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, topsep=1pt]
  \item \textbf{604 今日靶向}：多元函数微分学——二元 Taylor 展开与 Hessian 判别式 $\Delta = AC-B^2$、空间约束 Lagrange 乘数法求最值、法向量与梯度方向导数。
  \item \textbf{811 今日靶向}：一维对称方势阱（无限深宇称定理与 $\langle p \rangle = 0$；有限深画圆法与束缚态数目）；氢原子启航（两体约化、测不准原理支撑、最概然半径 $r_p=a_0$ 及玻尔球内几率积分）。
  \item \textbf{Spaced Retrieval 间隔提取 (Day +3 滚动回溯)}：\textbf{604 (Day 3)} 变上限积分拆项求导 $\frac{d}{dx}\int_0^x (x-t)f(t)dt = \int_0^x f(t)dt$；\textbf{811 (Day 2/3)} $\delta(x)$ 势阱导数跃变条件 $\Delta \psi'(0) = -\frac{2m\alpha}{\hbar^2}\psi(0)$ 与厚势垒 Gamow 穿透因子 $T\approx e^{-2\kappa a}$。
...
\checkbox\ Radial probability: $P(r) = \frac{4}{a_0^3}r^2 e^{-2r/a_0}$ ($r_p = a_0$) \quad
\checkbox\ Symmetric well energy: $E_n = \frac{n^2\pi^2\hbar^2}{8ma^2}$ \quad
\checkbox\ \alerttag{Day +3 604} $\frac{d}{dx}\int_0^x (x-t)f(t)dt = \int_0^x f(t)dt$ \quad
\checkbox\ \alerttag{Day +3 811} $\Delta \psi'(0) = -\frac{2m\alpha}{\hbar^2}\psi(0)$ \quad
\checkbox\ \alerttag{Day +3 811} Gamow factor $T \approx e^{-2\kappa a}$
\end{briefing}
...
\textcolor{mist}{\footnotesize\itshape 5--8 分钟闭卷盲写：默写今日二元极值判别式、氢原子径向几率 $P(r)$，并滚动回溯 3 天前变限求导拆项与 $\delta$ 势阱导数跃变。}
\vspace{9.0cm}
\end{tcolorbox}
...
\begin{tabularx}{\linewidth}{@{}p{2.4cm}X@{}}
  ...
  \midrule
  \textbf{Spaced Retrieval} & Day +3 Review: \checkbox\ 604 变限求导拆项 \quad \checkbox\ 811 $\delta$ 势导数跃变 \quad \checkbox\ 811 Gamow 因子\\
  \midrule
  \textbf{Summary} & \textbf{Total Score:} \underline{\hspace{1.4cm}} / 100 \quad\textbar\quad Total Time: \underline{\hspace{1.4cm}} min\\
  & \textbf{Next Spaced retrieval (Day +3):} 9月11日盲写 Hessian 判别法与氢原子径向几率 $P(r)$ 极值\\
\end{tabularx}
\end{debrief}
